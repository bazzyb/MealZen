...
import { TEST_FREE_EMAIL, TEST_FREE_ID_PREFIX, TEST_ID_PREFIX } from "@/consts";
import { TEST_BOOKS, TEST_MEALS } from "@/utils/testing/fixtures";

export async function buildTestUser(db: AbstractPowerSyncDatabase, userId: string, email?: string) {
  const prefix = email === TEST_FREE_EMAIL ? TEST_FREE_ID_PREFIX : TEST_ID_PREFIX;
  ...
  await db.writeTransaction(async tx => {
    ...
    for (let i = 0; i < TEST_BOOKS.length; i++) {
      const bookId = prefix + "00000-0000-0000-0000-000000000" + pad(String(i), 3, "0");
      bookIds.push(bookId);
    ...
    for (let i = 0; i < TEST_MEALS.length; i++) {
      const mealId = prefix + "00000-0000-0000-0000-000000000" + pad(String(i), 3, "0");
